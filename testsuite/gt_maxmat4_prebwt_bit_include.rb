...
def checkmaxmat4withmummer(reffile,queryfile,matchmode)
  ...
  if (matchmode=="mumreference") 
      ...
      run "sed -e '/^>/d' #{$last_stdout} | sort"  # was added later
			...
			run_test("#{$bin}gt dev maxmat4 -#{matchmode} -p -bit -r -l #{determinemaxmatchminlength(reffile)} -s -c #{pckname} #{queryfilepath}", :maxtime => 32000)
      run "sed -e '/^>/d' #{$last_stdout} | sort"  # was added later
  elsif (matchmode=="maxmatch") 
  		run_test("#{$bin}gt dev maxmat4 -#{matchmode} -p -bit -l #{determinemaxmatchminlength(reffile)} -L -s #{pckname} #{queryfilepath}", :maxtime => 32000)
  ...
  repfindtestfiles.each do |reffile|  
    repfindtestfiles.each do |queryfile|
      if reffile != queryfile				
        Name "maxmat4 -mumref -p -bit #{reffile}/#{queryfile}(m)"
        Keywords "check_maxmat4_mumreference_prebwt_bit_with_mummer"
        Test do
          checkmaxmat4withmummer(reffile,queryfile,"mumreference")
        end
        Name "maxmat4 -maxmatch -p -bit #{reffile}/#{queryfile}(m)"
        Keywords "check_maxmat4_maxmatch_prebwt_bit_with_mummer"
        Test do
          checkmaxmat4withmummer(reffile,queryfile,"maxmatch")
        end   
        Name "maxmat4 -mum -p -bit #{reffile}/#{queryfile}(m)"
        Keywords "check_maxmat4_mum_prebwt_bit_with_mummer"
        Test do
  ...
  allfiles.each do |queryfile|
    [12,15,18,21,24].each do |minlength| 
			if queryfile != reffile
				Name "maxmat4 -p -bit #{minlength} #{reffile}/#{queryfile}(r)"
				Keywords "check_maxmat4_maxmatch_prebwt_bit_with_repfind"
				Test do
